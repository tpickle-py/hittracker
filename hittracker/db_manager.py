...
class DatabaseManager:
    ...
    def add_firewall(self, name, device_type):
        self.cursor.execute('INSERT OR IGNORE INTO firewalls (name, device_type) VALUES (?, ?)', (name, device_type))
        self.conn.commit()
    ...
    def update_policy(self, firewall_name, policy_name, hit_count, date):
        ...
        if policy_data:
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            ''', (firewall_id, policy_name, hit_count, date, date if hit_count == 0 else None, date if hit_count == 0 else None))
    ...
    def get_unused_policies(self, days_threshold):
        ...
        AND julianday(?) - julianday(p.last_zero_hit) >= ?
